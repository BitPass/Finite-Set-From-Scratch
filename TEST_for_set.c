/* Driver for testing set functionality */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "list.h"
#include "set.h"


static int match_int(const void *pkey1, const void *pkey2)
{
        return !memcmp(pkey1, pkey2, sizeof(int));
}


static void print_set(const Set *pset)
{
        _Elem *pmember;
        int *pdata, size, i;
        fprintf(stdout, "Set size is %d\n", size = set_size(pset));
        i = 0;
        pmember = LIST_head(pset);
        while (i < size)
        {
                pdata = ELEM_data(pmember);
                fprintf(stdout, "set[%03d]=%3d\n", i, *pdata);
                pmember = ELEM_next(pmember);
                i++;
        }
        return;
}

int main(int argc, char **argv)
{
        Set set, set1, set2;    // actual sets, not pointers to sets
                                // (although they're often used with "&" operator so they may as well be)
        int *p_i, retval, i;    // p_i formerly data

        set_init(&set, match_int, free);

        fprintf(stdout, "Inserting 10 members\n");
        for (i = 9; i >= 0; i--)
        {
                if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                {
                        return 1;
                }
                *p_i = i + 1;
                if ((retval = set_insert(&set, p_i)) < 0)
                {
                        return 1;
                }
                else if (retval == 1)
                {
                        free(p_i);
                }
        }
        print_set(&set);
        fprintf(stdout, "Inserting the same members again\n");
        for (i = 9; i >= 0; i--)
        {
                if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                {
                return 1;
                }
                *p_i = i + 1;
                if ((retval = set_insert(&set, p_i)) < 0)
                {
                        return 1;
                }
                else if (retval == 1)
                {
                        free(p_i);
                }
        }
        print_set(&set);

        fprintf(stdout, "Inserting 200 and 100\n");
        if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                return 1;
        *p_i = 200;
        if ((retval = set_insert(&set, p_i)) < 0)
        {
                return 1;
        }
        else if (retval == 1)
                {
                        free(p_i);
                }
        if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                return 1;
        *p_i = 100;

        if ((retval = set_insert(&set, p_i)) < 0)
                return 1;
        else if (retval == 1)
        {
                free(p_i);
        }

        print_set(&set);

        fprintf(stdout, "Removing 100, 5, and 10\n");
        i = 100;
        p_i = &i;
        if (set_remove(&set, (void **)&p_i) == 0)
        {
                free(p_i);
        }
        i = 5;
        p_i = &i;
        if (set_remove(&set, (void **)&p_i) == 0)
        {
                free(p_i);
        }
        i = 10;
        p_i = &i;
        if (set_remove(&set, (void **)&p_i) == 0)
        {
                free(p_i);
        }

        print_set(&set);

        fprintf(stdout, "Removing three members\n");

        if (list_rem_next(&set, NULL, (void **)&p_i) == 0)
        {
                free(p_i);
        }

        if (list_rem_next(&set, NULL, (void **)&p_i) == 0)
        {
                free(p_i);
        }


        if (list_rem_next(&set, NULL, (void **)&p_i) == 0)
                free(p_i);

        print_set(&set);

        fprintf(stdout, "Removing all members\n");

        while (set_size(&set) > 0)
        {
                if (list_rem_next(&set, NULL, (void **)&p_i) == 0){
                        free(p_i);
                }
        }

        print_set(&set);

        /*  Initialize two sets.                                                      */
        set_init(&set1, match_int, free);
        set_init(&set2, match_int, free);

        /*  Perform some set operations.                                              */

        for (i = 9; i >= 0; i--)
        {
                if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                {
                        return 1;
                }
                *p_i = i + 1;
                if ((retval = set_insert(&set1, p_i)) < 0)
                        {
                                return 1;
                        }
                        else if (retval == 1)
                        {
                                free(p_i);
                        }
                if (i == 5 || i == 6 || i == 7)
                {
                        if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                        {
                                return 1;
                        }
                        *p_i = i * 10;
                        if ((retval = set_insert(&set2, p_i)) < 0)
                        {
                                return 1;
                        }
                                else if (retval == 1)
                                {
                                        free(p_i);
                                }
                }
                        else if (i == 3 || i == 1)
                        {
                                if ((p_i = (int *)malloc(sizeof(int))) == NULL)
                                {
                                        return 1;
                                }
                *p_i = i;
                if ((retval = set_insert(&set2, p_i)) < 0)
                {
                        return 1;
                }
                else if (retval == 1)
                {
                        free(p_i);
                }
                }
        }

        fprintf(stdout, "Set 1 for testing unions, intersections, and differences\n");
        print_set(&set1);
        fprintf(stdout, "Set 2 for testing unions, intersections, and differences\n");
        print_set(&set2);

        fprintf(stdout, "Determining the union of the two sets\n");

        if (set_union(&set, &set1, &set2) != 0)
                return 1;

        print_set(&set);

        fprintf(stdout, "Destroying the union\n");
        set_destroy(&set);

        fprintf(stdout, "Determining the intersection of the two sets\n");

        if (set_intersection(&set, &set1, &set2) != 0)
                return 1;

        print_set(&set);

        fprintf(stdout, "Testing whether the intersection equals Set 1...Value=%d "
                "(0=OK)\n", set_is_equal(&set, &set1));

        fprintf(stdout, "Testing whether Set 1 equals itself...Value=%d (1=OK)\n",
                set_is_equal(&set1, &set1));

        fprintf(stdout, "Testing whether the intersection is a subset of Set 1..."
                "Value=%d (1=OK)\n", set_is_subset(&set, &set1));

        fprintf(stdout, "Testing whether Set 2 is a subset of Set 1...Value=%d "
                "(0=OK)\n", set_is_subset(&set2, &set1));

        fprintf(stdout, "Destroying the intersection\n");
                set_destroy(&set);

        fprintf(stdout, "Determining the difference of the two sets\n");

        if (set_difference(&set, &set1, &set2) != 0)
                return 1;

        print_set(&set);

        i = 3;
        fprintf(stdout, "Testing whether %03d is in difference...Value=%d (0=OK)\n",i,
                set_is_member(&set, &i));
        i = 5;
        fprintf(stdout, "Testing whether %03d is in difference...Value=%d (1=OK)\n",i,
                set_is_member(&set, &i));
        /*  Destroy the sets.          */
        fprintf(stdout, "Destroying the sets\n");
        set_destroy(&set);
        set_destroy(&set1);
        set_destroy(&set2);
        return 0;
}
